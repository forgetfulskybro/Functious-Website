...
interface InviteButtonProps {
  size?: 'eesm' | 'esm' | 'sm' | 'md' | 'lg';
  className?: string;
}

const sizeClasses = {
  eesm: 'px-5 py-1 text-sm max-w-[165px]', 
  esm: 'px-3 py-1.5 text-sm',
  sm: 'px-5 py-2.5 text-sm',
  md: 'px-7 py-3.5 text-base',
  lg: 'px-9 py-4 text-lg',
} as const;

export default function InviteButton({ size = 'md', className = '' }: InviteButtonProps) {
  return (
    <a
      href={BOT_INVITE_URL}
      target="_blank"
      ...
      className={[
        ...
        'text-white shadow-xl shadow-orange-600/40',
        'border border-white/20',

        'hover:from-orange-600 hover:via-rose-600 hover:to-pink-600',
    ...
    >
      <svg 
        ...
        height="20" 
        fill="currentColor" 
        className="relative text-white/90 group-hover:text-white transition-colors"
        aria-hidden="true"
      >
      ...
      <span className="group-hover:text-white transition-colors">Add to Fluxer</span>
    </a>
  );
}