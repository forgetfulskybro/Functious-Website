...
function mapBypass(raw: unknown): BypassEntry[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((r: any) =>
      typeof r === 'string' || typeof r === 'number'
        ? { role: String(r), commands: [] as string[] }
        : {
            role: String(r.role ?? r.id ?? r.roleId ?? ''),
            commands: Array.isArray(r.commands)
              ? r.commands.map((c: any) => String(c).toLowerCase())
              : [],
          }
    )
    .filter((r) => r.role);
...
function mapJoinRoles(data: GuildData): JoinRoleEntry[] {
  const permanent = (data.joinRoles ?? [])
    .map((r: any) => ({
...
function mapSticky(raw: unknown): StickyEntry[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((r: any) => ({
      user: String(r.user ?? ''),
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-white/15 text-[10px] mt-1">
              Entries appear when members with roles leave.
            </p>
          </div>
        ) : (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-1">
                <p className="text-white/20 text-[10px]">
                  {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, entries.length)} of{' '}
                  {entries.length}
                </p>
...
export default function RolesClient({
  ...
  const hydratedForGuild = useRef<string | null>(null);

  useEffect(() => {
    hydratedForGuild.current = null;
  }, [activeGuildId]);

  useEffect(() => {
    if (loading) return;
    if (hydratedForGuild.current === activeGuildId) return;

    setBypass(mapBypass(data.bypassRoles));
    setJoinRoles(mapJoinRoles(data));
    setStickyEntries(mapSticky((data as any).stickyRoles));
    hydratedForGuild.current = activeGuildId;
  }, [loading, activeGuildId, data]);
  ...
  function persistJoinRoles(
    ...
    setJoinRoles((prev) => {
      const next =
        typeof entriesOrUpdater === 'function' ? entriesOrUpdater(prev) : entriesOrUpdater;
    ...
    <div className="min-h-screen bg-bg-dark flex">
      ...
      <main className="flex-1 px-6 py-8 max-w-5xl mx-auto w-full">
        ...
        <div className="rounded-xl bg-bg-card px-6 py-1 mb-5">
          {loading ? (
            ...
            <SettingRow
              label="Sticky Roles"
              description="Re-assign roles to members when they rejoin."
            >
              <Toggle
                value={data.stickyRolesEnabled}
                onChangeAction={(v) => handleSave({ stickyRolesEnabled: v })}
              />
            </SettingRow>
          )}
        </div>
              ...
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-white/20 text-[10px] mt-0.5">
                    Automatically assigned when a member joins
                  </p>
                </div>
                {!loading && (
                  <button
            ...
            <section className="rounded-2xl bg-bg-card p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-white/20 text-[10px] mt-0.5">
                    Bypass permission locks on commands
                  </p>
                </div>
                {!loading && (
                  <button
      ...
      {bypassModal && bypassModal !== 'create' && (
        <BypassModal
          ...
          onSave={(entry) => {
            setBypass((prev) => {
              const next = prev.map((b) => (b.role === bypassModal.role ? entry : b));
              handleSave({ bypassRoles: next as any });
              return next;
            });
          }}
